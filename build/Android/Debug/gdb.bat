:: This file was generated based on C:\ProgramData\Uno\Packages\UnoCore\0.35.8\Targets\Android\gdb.bat.
...
set NDK_DIR=C:\Users\ManailMJ\AppData\Local\Fusetools\Fuse\Android\AndroidNDK
...
set DATA_DIR=/data/data/com.apps.bimcast
set DEVICE_GDBSERVER=%DATA_DIR%/lib/gdbserver

set cmd="%ADB% shell ps | %AWK% -f %AWK_SCRIPTS%\extract-pid.awk -v PACKAGE=""com.apps.bimcast"""
FOR /F "delims=" %%i IN (' %cmd% ') DO SET PID=%%i

echo.
echo ## Starting gdbserver

start /b cmd /c %ADB% shell run-as com.apps.bimcast %DEVICE_GDBSERVER% +debug-socket --attach %PID%
timeout /t 1 > nul
